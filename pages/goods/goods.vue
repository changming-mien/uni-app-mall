<template>
	<view>
		...
		<view class="swiper-box">
			<swiper :autoplay="true" circular="true" @change="handleSwiper">
				<swiper-item v-for="(item,index) in goodsData.swiperList" :key="index">
		...
		<!-- 选择规格 -->
		<view class="info-box">
			<view class="row" @tap="specInfo.showSpec=true">
				<view class="text">
					选择
				</view>
				<view class="content">
		...
		<view class="footer">
			<view class="icons">
				...
				<view class="box" @tap="keep">
					<view class="icon iconfont" v-if="isKeep">
						&#xe64b;
					</view>
					<view class="icon iconfont" v-else>
						&#xe64c;
					</view>
					<view class="text">
						{{isKeep?'已':''}}收藏
					...
					success:(res=>{
						...
						goodsList.forEach(goods=>{
							if(goods.goods_id == this.goodsInfo.goods_id && goods.spec == this.goodsInfo.spec){
								// 如果存在修改商品数量
								let goodNumber=Number(goods.number)
								goodNumber += Number(this.goodsInfo.number);
								goods.number = goodNumber;
								isExist = true
							}
						})
						if(!isExist){
